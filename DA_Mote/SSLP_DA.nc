...
/********************************************EVENTS****************************************************************************/
	...
	event void UDPReceive.recvfrom(struct sockaddr_in6 *from, void *data, 
		...
		else if(header->msgid==SERVICE_TYPE_REQUEST)
		{
			...
			directory_advt_msg *dadv = (directory_advt_msg *)data;
			call Leds.led2Toggle();
			printf("Length_URL:%u",htons(dadv->location_entry.length_url));
			printf("URL:%s\n",dadv->location_entry.url);
			printf("Scope : %s",dadv->scope);
			printfflush();
		}
		else{
		//	call Leds.led0Toggle();
