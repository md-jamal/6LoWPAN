...
public class SSLPMessage {
	...
	public static byte[] parse(InetAddress address, int port,
		...
		System.out.println("Length of the pkt "+packet.getLength());
		...
		if(version==2)
		{
			...
			switch(MessageID)
			{
				case SERVICE_REQUEST:
					...
					byte[] length_stype = new byte[2];
					ByteBuffer buff = ByteBuffer.wrap(length_stype);
					byte[] length_st = buff.putShort(length_service_type).array();
					System.out.println("Length of the service type is "+length_st[0]+"  " +length_st[1]);
					byte[] service_type;
					if(length_service_type>0)
					{
						service_type=new byte[length_st[0]];		//as we dont have malloc at the tinyos we will read 16 bytes from the buffer which is fixed at the tinyos side
						buf.get(service_type);
					service=new String(service_type);
						//System.out.println("The service is"+service);	
						byte[] rem= new byte[24-length_st[0]];
						buf.get(rem);
				...
				case SERVICE_REGISTRATION:
					...
					lifetime = buf.getShort();
					System.out.println("The lifetime is"+lifetime);
					
					LT=buf.get();
					System.out.println("The value of LT is"+Integer.toBinaryString(LT));
							
					url_length=buf.getShort();
					System.out.println("The url length is "+url_length);	
					URL="";
					if(url_length>0)//TODO:has to do for more than one entries
					{
					...
					byte[] rem = new byte[80-url_length];
					buf.get(rem);

					System.out.println("rem:"+rem[0]+" "+rem[1]+" "+rem[2]+" "+rem[3]);

					int service_type_length=buf.getShort();
					System.out.println("The service type length is"+service_type_length);
					byte[] Service_Type;
					String Servicetype="";
					if(service_type_length>0)
					{
						Service_Type=new byte[service_type_length];
						buf.get(Service_Type);
						Servicetype = new String(Service_Type);
						System.out.println("The service type is"+Servicetype);
					}
					rem = new byte[24-service_type_length];
					buf.get(rem);
					
					short length_scope_type=buf.getShort();
					//System.out.println("The service type length is"+length_service_type);
					byte [] Scope_type;
					if(length_scope_type>0)
					{
						Scope_type=new byte[length_scope_type];
						buf.get(Scope_type);
						ScopeType = new String(Scope_type);
						//System.out.println("The service type is"+ServiceType);
					}

					//Its time to create a SLPServiceReply Message
					//First Create a SLP Header
					SLPHeader sregheader = new SLPHeader(SLPMessage.SERVICE_REGISTRATION,isOset(flags),isFset(flags),seq_no);
					try{
						ServiceURL Serviceurl = new ServiceURL(URL, lifetime);
						SLPServiceRegistration sreg = new SLPServiceRegistration(sregheader,Serviceurl,service_type_length,Servicetype,length_scope_type,ScopeType);
			...
			System.out.println("SSLP does not support version "+version);
		}
	...
	public static short getSequenceNumber(DatagramPacket packet)
	{
		...
		try{
			...
			ByteBuffer buf = ByteBuffer.wrap(packet.getData());
			buf.order(ByteOrder.LITTLE_ENDIAN);
			//read the first 16 bits as the next 16 bits is the short address
			buf.getShort();
