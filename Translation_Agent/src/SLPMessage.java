	...
	//	System.out.println("The sender address is "+senderAddress);
		...
		try {
			...
			if(version==2)
			{
				...
				System.out.println("Function id is"+Func_Id);
				...
				case SERVICE_ACKNOWLEDGE://As the service acknowledge is sent in response to sreg message this is not required
					...
					int Serviceackheader= Serviceack_header.getSSLPHeader();
					SSLPServiceAcknowledge sack = new SSLPServiceAcknowledge(Serviceackheader,error_code);

					return sack.getBytes();
				...
				case DA_ADVERTISEMENT:
					...
					int DA_Header= DAAdvt_header.getSSLPHeader();
					Error_code = in.readShort();
					//System.out.println("Error Code is "+Error_code);
					SSLPDirectoryAdvertisement DA_ADV = new SSLPDirectoryAdvertisement(DA_Header,Error_code,in);
					return DA_ADV.getBytes();
				...
				System.out.println("SLP Version: "+version+ "Not supported by Translation Agent");
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
