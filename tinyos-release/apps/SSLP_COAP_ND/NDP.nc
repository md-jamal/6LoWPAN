...
/*******************************************Tasks and Fn*************************************************************************/
  ...
  void fillPacket(uint16_t length,struct ip6_packet *packet,struct ip_iovec *v)
  {
	...
	packet->ip6_hdr.ip6_hlim=HOP_LIMIT;
	packet->ip6_data=v;
  ...
  void fillARO(aro_opt *addr_opt)
  {
	...
	if(call Node.getRouterState())
		addr_opt->reg_lifetime=10;	//5*60 Seconds=5 Minutes
	else
		addr_opt->reg_lifetime=5;	//10*60 seconds=10 Minutes
 ...
 void remainInterval()
 {
  ...
  void sendRA(struct in6_addr address)
  {
	...
	for(i=0;i<prefix_count;i++)
	{
		...
		length=sizeof(router_advt)+sizeof(stlla_opt)+sizeof(abro_opt)+sizeof(prefix_opt);
		v[0].iov_base=(uint8_t *)data;
		v[0].iov_len=length;
		v[0].iov_next=NULL;
		fillPacket(length,&pkt,&v[0]);
...
/*******************************************EVENTS*************************************************************************/
   ...
   event void IP_RA.recv(struct ip6_hdr *hdr, void *packet, 
	...
	memcpy(&TENTATIVE_ADDRESS,&prefix_option->prefix_information.prefix,prefix_option->prefix_information.prefix_length);
  ...
  event void NeighbrCache.prefixReg()
  {
	...
	#endif
	#ifdef NODE_ROUTER
		memcpy(&def_rtr,call RPLRoute.getDodagId(),sizeof(struct in6_addr));
	#endif
	RSSTATE=UNICAST;
	sendMsg(def_rtr,RS);
	call RSTimer.startOneShot(RSInterval);
