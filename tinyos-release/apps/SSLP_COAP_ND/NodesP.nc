...
{	
	...
	int allocindex()
	{
		...
		{
			//printf("%d length:%d",index,sa_services[index].lifetime);
			if(!sa_services[index].lifetime)
			{
				break;
			}
	...
	command error_t Nodes.setUA()
	{
		UAstate=TRUE;
		//printf("\n setting the Nodes as UA");
		return SUCCESS;
	}

	command error_t Nodes.getUAState()
	{
	...
	command error_t Nodes.setSA()
	{
		
		SAstate=TRUE;
		//printf("\n setting the Nodes as SA");
		call Timer.startPeriodic(60000U);		//Timer for 1 Minute
		return SUCCESS;
	}

	command error_t Nodes.getSAState()
	{
		return SAstate;
	}
	...
	command error_t Nodes.addService(char *serv,char *scop,char *url,uint16_t lifetime)
	{
		...
		else
		{
			...
			if(duplication(sa_services[index].service,sa_services[index].scope,sa_services[index].url))
			{
				...
				sa_services[index].service[stringlength(sa_services[index].service)]='\0';
				memcpy(&sa_services[index].scope,scop,stringlength(scop));
				sa_services[index].scope[stringlength(sa_services[index].scope)]='\0';
				memcpy(&sa_services[index].url,url,stringlength(url));
				sa_services[index].url[stringlength(sa_services[index].url)]='\0';
			}
			else{
			...
			//printf("\n service is %s,scop is %s,url is %s",serv,scop,url);
	...
	command error_t Nodes.removeService(char *service,char *scope,char *url)
	{
		uint8_t index;
		servicelocation_entry location_entry;
		index=findService(service);
	...
	command uint8_t Nodes.findService(char *service,char *scope)
	{
		int index;
		if(call Nodes.getSAState())
		{
			//printf("\n Nodes.findService:%s with scope:%s",service,scope);
	...
	command error_t Nodes.changeScope(char *service,char *new_scope)
	{
		if(call Nodes.getSAState())
	...
	command char * Nodes.getServices(char *scope,uint16_t len)
	{
		if(call Nodes.getSAState())
		{
	...
	command bool Nodes.servicesPresent(char *scope)
	{
		if(call Nodes.getSAState())
		{
			uint8_t i;
	...
	command error_t Nodes.getServiceURL(char *servicetype,char *scope,char *url)
	{
	...
	command error_t Nodes.printServices()
	{
		//int i;
		if(call Nodes.getSAState())
		{	
